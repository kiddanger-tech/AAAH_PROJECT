...
import { Leaf, Bug, Droplets, Thermometer, Wifi, WifiOff, Bell, BellOff, Settings, X, Power, RotateCw, Video, Play, Film, Gauge, Waves, Check, Sprout, Sliders } from 'lucide-react';
...
export default function AahhFieldConsole() {
  ...
  const [thresholdsDraft, setThresholdsDraft] = useState(null);
  ...
  const fetchThresholds = useCallback(async () => {
    if (!config.url || !config.anonKey) return;
    try {
      const res = await fetch(`${config.url}/rest/v1/thresholds?id=eq.1`, {
        headers: { apikey: config.anonKey, Authorization: `Bearer ${config.anonKey}` },
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      ...
      if (row) {
        setThresholds(row);
        setThresholdsDraft(prev => prev ?? row);
      }
    } catch (err) {
      // silent — thresholds are secondary to the main telemetry connection
    }
  }, [config]);

  const saveThresholds = async () => {
    if (!config.url || !config.anonKey || !thresholdsDraft) return;
    ...
    try {
      const res = await fetch(`${config.url}/rest/v1/thresholds?id=eq.1`, {
        ...
        body: JSON.stringify({
          moisture_low_pct: Number(thresholdsDraft.moisture_low_pct),
          moisture_high_pct: Number(thresholdsDraft.moisture_high_pct),
          water_level_low_pct: Number(thresholdsDraft.water_level_low_pct),
        }),
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      setThresholds(thresholdsDraft);
      pushLog('command', `Thresholds updated — moisture ${thresholdsDraft.moisture_low_pct}–${thresholdsDraft.moisture_high_pct}%, water level low ${thresholdsDraft.water_level_low_pct}%`);
      setThresholdsOpen(false);
    } catch (err) {
      pushLog('error', `Threshold save failed: ${err.message}`);
    } finally {
  ...
  }, [polling, fetchState, fetchClips, fetchDetections, fetchThresholds]);
    ...
    <div style={styles.page}>
      ...
      <header style={styles.header}>
        ...
        <div style={styles.headerRight}>
          ...
          {connected && (
            <button style={styles.iconBtn} onClick={() => { setThresholdsDraft(thresholds); setThresholdsOpen(true); }} aria-label="Thresholds">
              <Sliders size={18} color="#e9dcc8" />
            </button>
          )}
      ...
      {/* Thresholds panel */}
      {thresholdsOpen && thresholdsDraft && (
        <div style={styles.overlay} onClick={() => setThresholdsOpen(false)}>
          <div style={styles.configCard} onClick={e => e.stopPropagation()}>
            <div style={styles.configHead}>
              <span style={{ fontFamily: FONT_DISPLAY, fontSize: 20, fontWeight: 700, letterSpacing: 0.5 }}>
                Irrigation thresholds
              </span>
              <button style={styles.xBtn} onClick={() => setThresholdsOpen(false)} aria-label="Close">
                <X size={18} color="#e9dcc8" />
              </button>
            </div>
            <p style={styles.configHint}>
              Sets when the pump turns on/off and when you'll get a low-water alert. These sync to the
              sensor board, which reads them each cycle — no reflashing needed.
            </p>

            <label style={styles.label}>Pump ON below (%)</label>
            <input
              style={styles.input}
              type="number"
              min="0" max="100"
              value={thresholdsDraft.moisture_low_pct}
              onChange={e => setThresholdsDraft(d => ({ ...d, moisture_low_pct: e.target.value }))}
            />

            <label style={styles.label}>Pump OFF above (%)</label>
            <input
              style={styles.input}
              type="number"
              min="0" max="100"
              value={thresholdsDraft.moisture_high_pct}
              onChange={e => setThresholdsDraft(d => ({ ...d, moisture_high_pct: e.target.value }))}
            />

            <label style={styles.label}>Low water reservoir alert (%)</label>
            <input
              style={styles.input}
              type="number"
              min="0" max="100"
              value={thresholdsDraft.water_level_low_pct}
              onChange={e => setThresholdsDraft(d => ({ ...d, water_level_low_pct: e.target.value }))}
            />

            {Number(thresholdsDraft.moisture_low_pct) >= Number(thresholdsDraft.moisture_high_pct) && (
              <p style={{ fontSize: 11.5, color: '#ff9d7a', marginTop: -8, marginBottom: 14 }}>
                "Pump ON" should be lower than "Pump OFF", or the pump will cycle constantly.
              </p>
            )}

            <button
              style={styles.primaryBtn}
              disabled={thresholdsSaving || Number(thresholdsDraft.moisture_low_pct) >= Number(thresholdsDraft.moisture_high_pct)}
              onClick={saveThresholds}
            >
              <Sliders size={16} /> {thresholdsSaving ? 'Saving...' : 'Save thresholds'}
            </button>
          </div>
        </div>
      )}
