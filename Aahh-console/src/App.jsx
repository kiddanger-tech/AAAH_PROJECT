...
import { Leaf, Bug, Droplets, Thermometer, Wifi, WifiOff, Bell, BellOff, Settings, X, Power, RotateCw, Video, Play, Film } from 'lucide-react';
...
export default function AahhFieldConsole() {
  ...
  const [state, setState] = useState({ last_temp: null, last_hum: null, alarm_active: false, irrigation_active: false });
    ...
    <div style={styles.page}>
      ...
      <header style={styles.header}>
        ...
        <div style={styles.headerRight}>
          <ConnBadge connected={connected} error={lastError} />
...
function ConnBadge({ connected, error }) {
  return (
    <div style={{
      ...styles.connBadge,
      borderColor: connected ? '#6f8f5a' : '#8a5a45',
      color: connected ? '#a8c98f' : '#d99575',
    }}>
      {connected ? <Wifi size={14} /> : <WifiOff size={14} />}
      {connected ? 'ONLINE' : error ? 'OFFLINE' : 'NOT CONNECTED'}
    </div>
  );
}
...
function ReadingCard({ icon, label, value }) {
  return (
...
const styles = {
  ...
  statusGrid: {
    display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14, marginBottom: 30,
  },
  card: {
  ...
  cardValue: { fontFamily: FONT_DISPLAY, fontSize: 26, fontWeight: 700, marginTop: 'auto' },
  clearBtn: {
    border: 'none', borderRadius: 6, padding: '8px 0', fontSize: 11.5, fontWeight: 600,
    color: '#e9dcc8', background: '#3a3226', letterSpacing: 0.3,
  },
  logSection: { background: '#1c1710', border: '1px solid #3a3226', borderRadius: 10, padding: 18 },
